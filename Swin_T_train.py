...
#################### 模型加载
swin_tiny_patch4_window7_224 = torchvision.models.swin_t(weights=torchvision.models.Swin_T_Weights.DEFAULT)
class_cnt=10
swin_tiny_patch4_window7_224.head = torch.nn.Linear(768,class_cnt,bias=True)

# for name_temp,parameter_temp in swin_tiny_patch4_window7_224.named_parameters():
#     if name_temp.startswith('head'):
#         parameter_temp.requires_grad = True
#     else:
#         parameter_temp.requires_grad = False
# 
# print(swin_tiny_patch4_window7_224)
# for name_temp,_ in swin_tiny_patch4_window7_224.named_parameters():
...
warm_up_epochs = 5
warm_up_lr_start = 0.00001
warm_up_lr_end = learn_rate

optimizer = torch.optim.Adam( filter(lambda p: p.requires_grad, swin_model.parameters()),lr=learn_rate )
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=num_epochs-warm_up_epochs, eta_min=1e-6, last_epoch= -1, verbose=False)
...
for epoch in range(warm_up_epochs):
    ...
    if val_result_list[-1][2] > best_f1_score:
        ...
        best_swin = copy.deepcopy(swin_model)
...
for epoch in range(warm_up_epochs, num_epochs):
    ...
    if val_result_list[-1][2] > best_f1_score:
        best_f1_score = val_result_list[-1][2]
        best_resnet = copy.deepcopy(swin_model)


torch.save(best_swin, r'D:\work_files\task\best_swin.pth')
...
record.to_excel(r'D:\work_files\task\record_temp_swin.xlsx')